...
class AllImages:
    ...
    def add_image(self, imageString, rotationQuantities, TRBLedges):
        ...
        key = len(self.rotations)-1
        self.images[key] = []
        image = pg.transform.scale(pg.image.load(imageString), (self.size, self.size))
        for i in range(rotationQuantities):
...
class Tile:
    ...
    def set_random_superposition(self):
        ...
        self.finalImage = self.images.images[shape][rotation]
        self.collapsed = True
        self.drawTile = True
